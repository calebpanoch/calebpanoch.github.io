...
$(function () {
    ...
    startNewGame = function () {
        playing = true;
        turn = ["X", "O"][Math.floor(Math.random() * 2)]; //random x or o
        h2.html(turn+" goes first");
        score = {"X": 0, "O": 0};
        moves = 0;
        squares.forEach(function (square) {square.html(EMPTY);});
        squares.forEach(function (square) {square.addClass("pointer"); square.removeClass("defaultPointer");});
    },
    ...
    set = function () {
        ...
        $(this).addClass("defaultPointer");
        console.log($(this));
        moves += 1;
        score[turn] += $(this)[0].indicator;
        console.log(score[turn]);
        if (win(score[turn])) {
            h2.html("Player \""+turn+"\" wins!")
            playing = false;
            
            squares.forEach(function (square) {square.removeClass("pointer"); square.addClass("defaultPointer");});
     ...
     */
    play = function () {
        if (boardCreated) {startNewGame(); return;}
        boardCreated = true;
        var board = $("<table border=1 cellspacing=0>"), indicator = 1;
        board.addClass("tableCenter");
        ...
        $(document.getElementById("tictactoe")).append(board);// || document.body
        startNewGame();
    };
